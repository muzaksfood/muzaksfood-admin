...
;
        ...
        $flashDeals = $flashDeal->latest()->paginate($perPage)->appends($queryParam);

        return view('admin-views.offer.flash-deal-index', compact('flashDeals', 'search', 'perPage'));
    }
     ...
     * @return Factory|View|Application
     */
    public function addFlashSaleProduct(Request $request, $flash_deal_id): View|Factory|Application
    {
        $perPage = (int) $request->query('per_page', Helpers::getPagination());
        ...
        $flashDealProducts = $this->product->whereIn('id', $flashDealProductIds)->paginate($perPage)->appends($queryParam);

        $products = $this->product->active()->whereNotIn('id', $flashDealProductIds)->orderBy('name', 'asc')->get();

        return view('admin-views.offer.add-product-index', compact('flashDeal', 'flashDealProducts', 'products', 'perPage'));
    }
