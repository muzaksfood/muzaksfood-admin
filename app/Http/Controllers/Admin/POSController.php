...
class POSController extends Controller
{
    ...
    public function quickView(Request $request): JsonResponse
    {
        ...
        return response()->json([
            'success' => 1,
            'view' => view('admin-views.pos._quick-view-data', compact('product', 'discount', 'quantity', 'price', 'stock', 'buttonText'))->render(),
        ]);
    }
    ...
    public function quickViewModalFooter(Request $request): JsonResponse
    {
        ...
        return response()->json([
            'success' => 1,
            'view' => view('admin-views.pos.partials._quick-view-modal-footer', compact('product', 'discount', 'quantity', 'price', 'stock', 'buttonText'))->render(),
            'stock' => $stock
        ]);
    }
                ...
                foreach ($cart as $key => $cartItem) {
                    if (is_array($cartItem) && $cartItem['id'] == $request['id'] && $cartItem['variant'] == $str) {
                        $cart = $cart->map(function ($object) use ($request, $cartItem) {
                            if (!is_array($object)) {
                                Log::error('Cart item is not an array', ['object' => $object]);
                                return $object;
                            }

                            if ($object['id'] == $request->id && $object['variant'] == $cartItem['variant']) {
                                $object['quantity'] = (int) $request->quantity;
                            }
    ...
    public function orderList(Request $request)
    {
        ...
        $queryParam = ['branch_id' => $branchId, 'start_date' => $startDate, 'end_date' => $endDate, 'per_page' => $perPage];
        ...
        $orders = $query->orderBy('id', 'desc')->paginate($perPage)->appends($queryParam);

        return view('admin-views.pos.order.list', compact('orders', 'search', 'branches', 'branchId', 'startDate', 'endDate', 'perPage'));
    }
