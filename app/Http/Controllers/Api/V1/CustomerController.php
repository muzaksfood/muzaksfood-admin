...
class CustomerController extends Controller
{
    ...
    public function info(Request $request): JsonResponse
    {
        ...
        if (!isset($customer)){
            return response()->json(['message' => \App\CentralLogics\translate('Customer not found')], 404);
        }

        $lastIncompleteOfflineOrder = $this->order
            ->where(['user_id' => $request->user()->id, 'is_guest' => 0])
            ->where('payment_method', 'offline_payment')
            ->whereNotIn('order_status', ['delivered', 'canceled', 'failed', 'returned'])
            ->whereDoesntHave('offline_payment')
            ->with(['offline_payment', 'partial_payment'])
            ->orderBy('id', 'DESC')
            ->first();

        $customer->last_incomplete_offline_booking = $lastIncompleteOfflineOrder;
