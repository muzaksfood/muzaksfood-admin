...
class StoreOrderRequest extends FormRequest
{
    ...
    public function withValidator($validator) {
        $validator->after(function ($validator)  {
            $isExpress = (bool)($this->input('is_express', 0));
            
            // Skip time slot validation for express orders
            if (!$isExpress && !$this->input('time_slot_id')) {
                $validator->errors()->add('time_slot_id', translate('Time slot is required for regular orders'));
            }
