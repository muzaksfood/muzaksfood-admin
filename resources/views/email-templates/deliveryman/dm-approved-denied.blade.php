...
<head>
    ...
    <title>{{ translate('Deliveryman Registration Request Status') }}</title>
        ...
        <tr>
            <td style="padding:30px 30px 0">
                @if($dm->application_status == 'approved')
                    <img src="{{asset('/public/assets/admin/img/email-templates/approved.png')}}" alt="approved">
                    <h3 style="font-size:17px;font-weight:500">{{ translate('Registration Approved !') }}</h3>
                @endif
                @if($dm->application_status == 'denied')
                   <img src="{{asset('/public/assets/admin/img/email-templates/denied.png')}}" alt="denied">
                   <h3 style="font-size:17px;font-weight:500">{{ translate('Registration Denied !') }}</h3>
                @endif
            </td>
        </tr>
        <tr>
            <td style="padding:0 30px 30px; text-align:left">
                ...
                @if($dm->application_status == 'approved')
                    <span style="display:block;margin-bottom:14px">{{ translate('Your registration request has been approved by admin. Now you can login to MuzaksFood deliveryman app and view all the options') }}</span>
                @endif
                @if($dm->application_status == 'denied')
