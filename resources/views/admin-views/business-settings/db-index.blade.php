                ...
                <form action="{{route('admin.business-settings.web-app.system-setup.clean-db')}}" method="post"
                            ...
                            <div class="check-item">
                                <div class="form-group form-check form--check">
                                    ...
                                    <label class="form-check-label text-dark"
                                        for="{{$table['name']}}">{{ translate(Str::limit($table['name'], 20)) }}
                                    <span class="badge-pill badge-secondary mx-2">{{$table['rows']}}</span></label>
                                </div>
                            </div>
                        @endforeach
                    </div>
                    <div class="btn--container justify-content-end">
                        ...
                        <button type="{{env('APP_MODE')!='demo'?'submit':'button'}}"
                            class="btn btn-primary mb-2 call-demo">{{translate('clean')}}</button>
                    </div>
                </form>
            </div>
        </div>
