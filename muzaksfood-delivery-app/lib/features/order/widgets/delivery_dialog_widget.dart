...
class DeliveryDialogWidget extends StatelessWidget {
  ...
  const DeliveryDialogWidget({super.key, required this.onTap, this.totalPrice, this.orderModel});
                            ...
                            Provider.of<OrderProvider>(context, listen: false).updateOrderStatus(
                              ...
                              if (value.isSuccess) {
                                order.updatePaymentStatus(
                                    token: Provider.of<AuthProvider>(Get.context!, listen: false).getUserToken(), orderId: orderModel!.id, status: 'paid');
                                Provider.of<OrderProvider>(Get.context!, listen: false).getAllOrders();
                                Navigator.pop(context);
                                Navigator.of(Get.context!).pushReplacement(MaterialPageRoute(builder: (_) => OrderDeliveredScreen(orderID: orderModel!.id.toString())));
                              }
                            });
                          },
                        );
